...
def _decimal_room() -> tuple[Room, list[str]]:
    ...
    room = Room(code="ROOM01", deck=("0", "0.5", "1", "2"))
...
def test_revealing_a_decimal_round_does_not_raise():
    ...
    # reveal in a room holding `0.5` raised ValueError — a 500, not a domain error,
    # and only on reveal, so such a room looked fine right up until it wasn't.
    room, (host, a, b) = _decimal_room()
    room.cast_vote(a, "0.5")
    room.cast_vote(b, "2")
    room.reveal(host)
    results = room.results()
    assert results is not None
    assert results.votes == {a: "0.5", b: "2"}
...
def test_average_over_decimal_cards():
    ...
    room.cast_vote(a, "0.5")
    room.cast_vote(b, "1")
    assert _reveal(room, host).average == 0.75
